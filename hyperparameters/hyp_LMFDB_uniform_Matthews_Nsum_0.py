...
hyperparameters = dict(
    model_name="LMFDB_uniform_Matthews_Nsum_0_p1000",
    model_num=None,
    model_type="SimpleFCClassificationModel",
    model=dict(
        nonlin_str="ReLU_inplace",
        hidden_lin_ftrs="OPTIMIZE: lambda idx: int(2**idx) # discrete # tuple(range(3, 9))",
        hidden_lin_ftrs_no="OPTIMIZE: lambda idx: int(idx) # discrete # tuple(range(1, 6))",
        dropout="OPTIMIZE: lambda idx: float(idx) # continuous # (0.0, 0.9)",
    ),
    dataloader=dict(
