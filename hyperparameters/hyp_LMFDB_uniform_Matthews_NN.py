...
hyperparameters = dict(
    model_name="LMFDB_uniform_Matthews_NN_p1000",
    model_num=None,
    # model_type="SimpleFCClassificationModel",
    model_type="SimpleConvolutionalClassificationModel",
    model=dict(
        dim="OPTIMIZE: lambda idx: int(2**idx) # discrete # tuple(range(2, 7))",
        input_layers="OPTIMIZE: lambda idx: int(idx) # discrete # tuple(range(4))",
        reducing_layers=reducing_layers,
        output_layers="OPTIMIZE: lambda idx: int(idx) # discrete # tuple(range(4))",
        reduction_exponent=1.0,
        kernel_size="OPTIMIZE: lambda idx: int(1 + 2**idx) # discrete # tuple(range(1, 6))",
        stride=stride,
    ...
    epochs="OPTIMIZE: lambda idx: int(idx) # discrete # tuple(range(1, 11))",
    optimizer=dict(
        lr_max="OPTIMIZE: lambda idx: float(10 ** idx) # continuous # (-4, -1)",
        wd="OPTIMIZE: lambda idx: float(10 ** idx) # continuous # (-6, 0)",
    ),
)
